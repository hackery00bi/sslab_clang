...
# 11 "src/wrk.h" 2
# 1 "src/http_parser.h" 1
# 66 "src/http_parser.h"
typedef struct http_parser http_parser;
typedef struct http_parser_settings http_parser_settings;
# 83 "src/http_parser.h"
typedef int (*http_data_cb) (http_parser*, const char *at, size_t length);
typedef int (*http_cb) (http_parser*);
# 120 "src/http_parser.h"
enum http_method
  {

  HTTP_DELETE = 0, HTTP_GET = 1, HTTP_HEAD = 2, HTTP_POST = 3, HTTP_PUT = 4, HTTP_CONNECT = 5, HTTP_OPTIONS = 6, HTTP_TRACE = 7, HTTP_COPY = 8, HTTP_LOCK = 9, HTTP_MKCOL = 10, HTTP_MOVE = 11, HTTP_PROPFIND = 12, HTTP_PROPPATCH = 13, HTTP_UNLOCK = 14, HTTP_REPORT = 15, HTTP_MKACTIVITY = 16, HTTP_CHECKOUT = 17, HTTP_MERGE = 18, HTTP_MSEARCH = 19, HTTP_NOTIFY = 20, HTTP_SUBSCRIBE = 21, HTTP_UNSUBSCRIBE = 22, HTTP_PATCH = 23, HTTP_PURGE = 24,
...
inline static void tinymt64_next_state(tinymt64_t * random) {
    ...
    random->status[0] &= 0x7fffffffffffffffUL;
...
inline static double tinymt64_generate_double(tinymt64_t * random) {
    tinymt64_next_state(random);
    return tinymt64_temper(random) * (1.0 / 18446744073709551616.0);
}
...
int main(int argc, char **argv) {
    ...
    pthread_mutex_init(&statistics.mutex, ((void*)0));
    statistics.latency = stats_alloc(100000);
    statistics.requests = stats_alloc(100000);
    ...
    for (uint64_t i = 0; i < cfg.threads; i++) {
        ...
        if (pthread_create(&t->thread, ((void*)0), &thread_main, t)) {
            char *msg = strerror((*__errno_location ()));
            fprintf(stderr, "unable to create thread %""l" "u"" %s\n", i, msg);
            exit(2);
        }
    }
    ...
    printf("Making %""l" "u"" requests to %s\n", cfg.requests, url);
    printf("  %""l" "u"" threads and %""l" "u"" connections\n", cfg.threads, cfg.connections);
    ...
    printf("  %""l" "u"" requests in %s, %sB read\n", complete, runtime_msg, format_binary(bytes));
...
void *thread_main(void *arg) {
    ...
    aeCreateTimeEvent(loop, 100, sample_rate, thread, ((void*)0));
    aeCreateTimeEvent(loop, 2000, check_timeouts, thread, ((void*)0));
...
static int connect_socket(thread *thread, connection *c) {
    ...
    flags = 1;
    setsockopt(fd, IPPROTO_TCP, 1, &flags, sizeof(flags));

    if (aeCreateFileEvent(loop, fd, 2, socket_writeable, c) != 0) {
        goto error;
    }
...
static int reconnect_socket(thread *thread, connection *c) {
    aeDeleteFileEvent(thread->loop, c->fd, 2 | 1);
    close(c->fd);
...
static int sample_rate(aeEventLoop *loop, long long id, void *data) {
    ...
    return 100 + rand64(&thread->rand, 100);
}
...
static int request_complete(http_parser *parser) {
    ...
    aeDeleteFileEvent(thread->loop, c->fd, 1);
    aeCreateFileEvent(thread->loop, c->fd, 2, socket_writeable, c);
...
static void socket_writeable(aeEventLoop *loop, int fd, void *data, int mask) {
    ...
    aeCreateFileEvent(loop, fd, 1, socket_readable, c);
...
static int parse_args(struct config *cfg, char **url, char **headers, int argc, char **argv) {
    ...
    while ((c = getopt_long(argc, argv, "t:c:r:E:H:v?", longopts, ((void*)0))) != -1) {
        switch (c) {
            case 't':
            ...
            case 'H':
                *header++ = optarg;
                break;
            case 'v':
                printf("wrk %s [%s] ", "1.2.0", aeGetApiName());
...
static void print_units(long double n, char *(*fmt)(long double), int width) {
    ...
    if (((*__ctype_b_loc ())[(int) ((msg[len-1]))] & (unsigned short int) _ISalpha)) pad--;
    if (((*__ctype_b_loc ())[(int) ((msg[len-2]))] & (unsigned short int) _ISalpha)) pad--;
    width -= pad;
