...
# 1 "./commit-slab-decl.h" 1
# 5 "./commit-slab.h" 2
# 1 "./commit-slab-impl.h" 1
# 6 "./commit-slab.h" 2
# 12 "./commit.h" 2
...
void clear_alloc_state(struct alloc_state *s)
{
 ...
 do { free(s->slabs); (s->slabs) = ((void*)0); } while (0);
}
...
static inline void *alloc_node(struct alloc_state *s, size_t node_size)
{
 ...
 if (!s->nr) {
  s->nr = 1024;
  s->p = xmalloc(1024 * node_size);

  do { if ((s->slab_nr + 1) > s->slab_alloc) { if ((((s->slab_alloc)+16)*3/2) < (s->slab_nr + 1)) s->slab_alloc = (s->slab_nr + 1); else s->slab_alloc = (((s->slab_alloc)+16)*3/2); (s->slabs) = xrealloc((s->slabs), st_mult(sizeof(*(s->slabs)), (s->slab_alloc))); } } while (0);
...
void alloc_report(struct repository *r)
{
 report("blob", r->parsed_objects->blob_state->count, r->parsed_objects->blob_state->count * sizeof(struct blob) >> 10);
 report("tree", r->parsed_objects->tree_state->count, r->parsed_objects->tree_state->count * sizeof(struct tree) >> 10);
 report("commit", r->parsed_objects->commit_state->count, r->parsed_objects->commit_state->count * sizeof(struct commit) >> 10);
 report("tag", r->parsed_objects->tag_state->count, r->parsed_objects->tag_state->count * sizeof(struct tag) >> 10);
 report("object", r->parsed_objects->object_state->count, r->parsed_objects->object_state->count * sizeof(union any_object) >> 10);
}
