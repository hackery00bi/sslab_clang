# 1 "host/buildvm_asm.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 341 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "host/buildvm_asm.c" 2
...
err:
  ...
  if (strncmp(sym+(*sym == '_'), "lj_", sizeof("lj_")-1)) {
 ...
 fprintf(ctx->fp, "#endif\n");
      return;
    } else if (0 && ctx->mode == BUILD_machasm) {
...
# 287 "host/buildvm_asm.c"
  for (i = rel = 0; i < ctx->nsym; i++) {
    int32_t ofs = ctx->sym[i].ofs;
    int32_t next = ctx->sym[i+1].ofs;
# 300 "host/buildvm_asm.c"
