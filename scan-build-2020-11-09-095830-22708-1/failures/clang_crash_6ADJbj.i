...
__attribute__((__noreturn__)) void unable_to_lock_die(const char *path, int err);
# 233 "./lockfile.h"
         ...
         const void *keydata)
{
 ...
 pg1 = ((const struct packed_git *) ((char *)(entry) - __builtin_offsetof(const struct packed_git, packmap_ent)));
 pg2 = ((const struct packed_git *) ((char *)(entry2) - __builtin_offsetof(const struct packed_git, packmap_ent)));
        ...
        int create)
{
 ...
 pos = -pos-1;
 do { if ((it->subtree_nr + 1) > it->subtree_alloc) { if ((((it->subtree_alloc)+16)*3/2) < (it->subtree_nr + 1)) it->subtree_alloc = (it->subtree_nr + 1); else it->subtree_alloc = (((it->subtree_alloc)+16)*3/2); (it->down) = xrealloc((it->down), st_mult(sizeof(*(it->down)), (it->subtree_alloc))); } } while (0);
 it->subtree_nr++;

 do { size_t flex_array_len_ = (pathlen); (down) = xcalloc(1, st_add(st_add((sizeof(*(down))),(flex_array_len_)),(1))); memcpy((void *)(down)->name, (path), flex_array_len_); } while (0);
 down->cache_tree = ((void*)0);
 down->namelen = pathlen;

 if (pos < it->subtree_nr)
  move_array((it->down + pos + 1), (it->down + pos), (it->subtree_nr - pos - 1), sizeof(*(it->down + pos + 1)) + (sizeof(char [1 - 2*!(sizeof(*(it->down + pos + 1)) == sizeof(*(it->down + pos)))]) - 1));
...
static int do_invalidate_path(struct cache_tree *it, const char *path)
{
 ...
 if (!*slash) {
  ...
  if (0 <= pos) {
   ...
   move_array((it->down + pos), (it->down + pos + 1), (it->subtree_nr - pos - 1), sizeof(*(it->down + pos)) + (sizeof(char [1 - 2*!(sizeof(*(it->down + pos)) == sizeof(*(it->down + pos + 1)))]) - 1));
        ...
        int flags)
{
 ...
 int missing_ok = flags & 1;
 int dryrun = flags & 4;
 int repair = flags & 16;
 int to_invalidate = 0;
 int i;

 ((void) sizeof ((!(dryrun && repair)) ? 1 : 0), __extension__ ({ if (!(dryrun && repair)) ; else __assert_fail ("!(dryrun && repair)", "cache-tree.c", 253, __extension__ __PRETTY_FUNCTION__); }));
 ...
 *skip_count = 0;
 ...
 while (i < entries) {
  ...
  ce_missing_ok = mode == 0160000 || missing_ok ||
   (has_promisor_remote() &&
    ((ce)->ce_flags & (1 << 30)));
  if (is_null_oid(oid) ||
   ...
   if (expected_missing)
    return -1;
   return (error("invalid object %06o %s for '%.*s'", mode, oid_to_hex(oid), entlen+baselen, path), const_error());
  ...
  strbuf_add(&buffer, oid->hash, the_repository->hash_algo->rawsz);
 ...
 if (repair) {
  struct object_id oid;
  hash_object_file(the_repository->hash_algo, buffer.buf, buffer.len,
     tree_type, &oid);
  if (repo_has_object_file_with_flags(the_repository, &oid, 32))
   oidcpy(&it->oid, &oid);
  else
   to_invalidate = 1;
 } else if (dryrun) {
...
int cache_tree_update(struct index_state *istate, int flags)
{
 ...
 do { if (trace_pass_fl(&trace_perf_key)) trace_performance_leave_fl("cache-tree.c", 446, getnanotime(), "cache_tree_update"); } while (0);
 if (i < 0)
  return i;
 istate->cache_changed |= (1 << 5);
 return 0;
...
# 478 "cache-tree.c"
 if (0 <= it->entry_count) {
  strbuf_add(buffer, it->oid.hash, the_repository->hash_algo->rawsz);
 }
 for (i = 0; i < it->subtree_nr; i++) {
...
static struct cache_tree *read_one(const char **buffer, unsigned long *size_p)
{
 ...
 int i, subtree_nr;
 const unsigned rawsz = the_repository->hash_algo->rawsz;
...
struct tree* write_in_core_index_as_tree(struct repository *repo) {
 ...
 ret = write_index_as_tree_internal(&o, index_state, was_valid, 0, ((void*)0));
 if (ret == (-2)) {
  int i;
  ...
  for (i = 0; i < index_state->cache_nr; i++) {
   ...
   if ((((0x3000) & (ce)->ce_flags) >> 12))
    fprintf(stderr, "BUG: %d %.*s\n", (((0x3000) & (ce)->ce_flags) >> 12),
     (int)((ce)->ce_namelen), ce->name);
  }
  BUG_fl("cache-tree.c", 662, "unmerged index entries when writing inmemory index");
 }
...
int write_index_as_tree(struct object_id *oid, struct index_state *index_state, const char *index_path, int flags, const char *prefix)
{
 ...
 struct lock_file lock_file = { ((void*)0) };
 int ret;

 hold_lock_file_for_update(&lock_file, index_path, 1);
 ...
 ret = write_index_as_tree_internal(oid, index_state, was_valid, flags,
        prefix);
 if (!ret && !was_valid) {
  write_locked_index(index_state, &lock_file, (1 << 0));
         ...
         struct strbuf *path)
{
 ...
 struct strbuf tree_buf = { .alloc = 0, .len = 0, .buf = strbuf_slopbuf };
 ...
 while (i < it->entry_count) {
  ...
  if (ce->ce_flags & ((0x3000) | (1 << 29) | (1 << 17)))
   BUG_fl("cache-tree.c", 813, "%s with flags 0x%x should not be in cache-tree", ce->name, ce->ce_flags);
  ...
  if (slash) {
   ...
   if (!sub || sub->cache_tree->entry_count < 0)
    BUG_fl("cache-tree.c", 820, "bad subtree '%.*s'", entlen, name);
   oid = &sub->cache_tree->oid;
   mode = 0040000;
 ...
 if (!oideq(&new_oid, &it->oid))
  BUG_fl("cache-tree.c", 838, "cache-tree for path %.*s does not match. " "Expected %s got %s", len, path->buf, oid_to_hex(&new_oid), oid_to_hex(&it->oid));
...
{
 struct strbuf path = { .alloc = 0, .len = 0, .buf = strbuf_slopbuf };
