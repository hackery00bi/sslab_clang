...
static char *unsupported_term[] = {"dumb","cons25","emacs",((void*)0)};
static linenoiseCompletionCallback *completionCallback = ((void*)0);
static linenoiseHintsCallback *hintsCallback = ((void*)0);
static linenoiseFreeHintsCallback *freeHintsCallback = ((void*)0);
...
static int enableRawMode(int fd) {
    ...
    raw.c_iflag &= ~(0000002 | 0000400 | 0000020 | 0000040 | 0002000);
    ...
    raw.c_lflag &= ~(0000010 | 0000002 | 0100000 | 0000001);


    raw.c_cc[6] = 1; raw.c_cc[5] = 0;


    if (tcsetattr(fd,2,&raw) < 0) goto fatal;
    rawmode = 1;
    return 0;
...
static void disableRawMode(int fd) {
    ...
    if (rawmode && tcsetattr(fd,2,&orig_termios) != -1)
        rawmode = 0;
}
...
static int getColumns(int ifd, int ofd) {
    ...
    if (ioctl(1, 0x5413, &ws) == -1 || ws.ws_col == 0) {
...
void linenoisePrintKeyCodes(void) {
    ...
    while(1) {
        ...
        printf("'%c' %02x (%d) (type quit to exit)\n",
            ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISprint) ? c : '?', (int)c, (int)c);
        printf("\r");
        fflush(stdout);
    }
    disableRawMode(0);
...
static int linenoiseRaw(char *buf, size_t buflen, const char *prompt) {
    ...
    if (enableRawMode(0) == -1) return -1;
    count = linenoiseEdit(0, 1, buf, buflen, prompt);
    disableRawMode(0);
    printf("\n");
    return count;
}
...
char *linenoise(const char *prompt) {
    ...
    if (!isatty(0)) {
        ...
        printf("%s",prompt);
        fflush(stdout);
        if (fgets(buf,4096,stdin) == ((void*)0)) return ((void*)0);
        len = strlen(buf);
        while(len && (buf[len-1] == '\n' || buf[len-1] == '\r')) {
            len--;
            buf[len] = '\0';
...
int linenoiseHistorySave(const char *filename) {
    mode_t old_umask = umask(0100|((0400|0200|0100) >> 3)|(((0400|0200|0100) >> 3) >> 3));
    FILE *fp;
    int j;
