...
__attribute__((noinline)) void lj_err_argtype(lua_State *L, int narg, const char *xname)
{
  ...
  const char *tname = o < L->top ? (lj_obj_itypename[((((o)->it) <= 0xfffeffffu) ? ~(~13u) : (((int32_t)((o)->it) >> 15) == -2) ? ~(~3u) : ~((o)->it))]) : lj_obj_typename[0];
  const char *msg = lj_str_pushf(L, (lj_err_allmsg+(int)(LJ_ERR_BADTYPE)), xname, tname);
  err_argmsg(L, narg, msg);
}
